...
(function () {
    const KEY = 'copyAuth:v5'; if (window[KEY]) return; window[KEY] = true;
    ...
    if (typeof GM_registerMenuCommand !== 'undefined') {
        const updateMenu = () => {
            GM_registerMenuCommand('剪切板设定 🛠️', () => {
                ...
                mask.className = 'auth-settings-mask'; panel.className = 'auth-settings-panel';
                const authMode = (typeof GM_getValue !== 'undefined' ? GM_getValue('clipboard_auth_mode_' + location.host, 0) : 0);
                const css = `
                    ...
                    .btn-status { background:#fff; border:none; }
                    ...
                    @media (prefers-color-scheme: dark) {
                        ...
                        .btn-status { background:rgba(255,255,255,0.05) !important; }
                        .btn-close { color:#aaa; }
                ...
                const updateStatusUI = (mode) => {
                    const modes = { 0: ['监控中', '#30d158'], 1: ['禁止写入', '#FF3B30'], 2: ['自由复制', '#007AFF'] };
                    const isDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
                    btnStatus.innerHTML = `<span>复制授权</span> <small style="font-weight:700">${modes[mode][0]}</small>`;
                    btnStatus.style.color = modes[mode][1];
                    btnStatus.style.boxShadow = `0 4px 12px ${modes[mode][1]}22`;
                    if (mode === 1) {
                        enabled = false; freeCopyMode = false;
                        disableAllCopyAPIs();
                        if(dot) { dot.remove(); dot = null; }
                        hideAllModals();
                    } else if (mode === 2) {
                        enabled = true; freeCopyMode = true;
                        restoreAllCopyAPIs();
                        showDot();
                    } else {
                        enabled = true; freeCopyMode = false;
                        restoreAllCopyAPIs();
                        hookAPI();
                        hookExec();
                    }
                };
                updateStatusUI(authMode);
                btnStatus.onclick = (e) => {
                    e.stopPropagation();
                    const currentMode = (typeof GM_getValue !== 'undefined' ? GM_getValue('clipboard_auth_mode_' + location.host, 0) : 0);
                    const nextMode = (currentMode + 1) % 3;
                    if (typeof GM_setValue !== 'undefined') GM_setValue('clipboard_auth_mode_' + location.host, nextMode);
                    updateStatusUI(nextMode);
                };
                ...
                const unlockMode = (typeof GM_getValue !== 'undefined' ? GM_getValue('clipboard_unlock_mode_' + location.host, 1) : 1);
                const btnUnlock = document.createElement('button');
                btnUnlock.className = 'btn-status';
                const updateUnlockUI = (mode) => {
                    const modes = { 0: ['已禁用', '#FF3B30'], 1: ['温和模式', '#30d158'], 2: ['强力模式', '#FF9500'] };
                    const isDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
                    btnUnlock.innerHTML = `<span>解除复制限制</span> <small style="font-weight:700">${modes[mode][0]}</small>`;
                    btnUnlock.style.color = modes[mode][1];
                    btnUnlock.style.boxShadow = `0 4px 12px ${modes[mode][1]}22`;
                };
                updateUnlockUI(unlockMode);
                btnUnlock.onclick = (e) => {
                    e.stopPropagation();
                    const currentMode = (typeof GM_getValue !== 'undefined' ? GM_getValue('clipboard_unlock_mode_' + location.host, 1) : 1);
                    const nextMode = (currentMode + 1) % 3;
                    if (typeof GM_setValue !== 'undefined') GM_setValue('clipboard_unlock_mode_' + location.host, nextMode);
                    updateUnlockUI(nextMode);
                    toggleUnlock(nextMode);
                };
        ...
        const handleTap = () => {
           ...
           timer = setTimeout(() => {
            if (taps === 1) {
                ...
                else if (freeCopyMode) { freeCopyMode = false; hookAPI(); hookExec(); }
                else { enabled = true; denyCount = 0; restoreAllCopyAPIs(); hookAPI(); hookExec(); }
            } else if (taps === 2) { 
                freeCopyMode = enabled = true; denyCount = allowCount = 0; restoreAllCopyAPIs();
             }
             updateDot(); taps = 0;
         }, 300);
      };
    ...
    const hookExec = () => {
        const wrapExec = function(cmd, ui, val) {
            if (cmd === 'copy') {
                const sel = window.getSelection().toString();
                if (!sel) return originalExec.apply(document, arguments);
                return handleAuth(sel, () => {
                    const ta = document.createElement('textarea'); ta.value = sel; ta.style.cssText = 'position:fixed;opacity:0;';
                    document.body.appendChild(ta); ta.select(); originalExec.call(document, 'copy'); ta.remove();
                }), false;
            }
            return originalExec.apply(document, arguments);
        };
        try {
            Object.defineProperty(document, 'execCommand', { get: () => wrapExec, set: v => { if(v !== wrapExec) console.log('拦截到 execCommand 修改尝试') }, configurable: true });
        } catch (e) { document.execCommand = wrapExec; }
    };
    ...
    const toggleUnlock = (mode) => {
        const cssId = 'unlock-css-base';
        const oldCss = document.getElementById(cssId);
        if (oldCss) oldCss.remove();
        const evs = ['copy', 'cut', 'contextmenu', 'selectstart', 'mousedown', 'beforecopy', 'dragstart'];
        if (mode !== 1 && mode !== 2 && window._origPreventDefault) {
            Event.prototype.preventDefault = window._origPreventDefault;
            delete window._origPreventDefault;
        }
        if (mode === 0) return;
        ...
        if (!window._origPreventDefault) {
            window._origPreventDefault = Event.prototype.preventDefault;
            Event.prototype.preventDefault = function() {
                if (evs.includes(this.type)) return;
                return window._origPreventDefault.apply(this, arguments);
            };
        }
        evs.forEach(n => {
            try { Object.defineProperty(document, 'on' + n, { get: () => null, set: () => {}, configurable: true }); } catch (e) {}
            if (mode === 2) {
    ...
    const setupFrame = iframe => {
        try {
            const doc = iframe.contentDocument, win = doc?.defaultView, nav = win?.navigator;
            if (!doc || !win) return;
            ...
            const def = (o, p, v, m) => { try { Object.defineProperty(o, p, { get: () => v, set: n => { if(n !== v) console.log(m) }, configurable: true }); } catch (e) { o[p] = v; } };
    ...
    const checkAndFix = () => {
        ...
        try {
            if (navigator.clipboard) {
                const c = navigator.clipboard, t = c.writeText?.toString() || '', w = c.write?.toString() || '';
                if ((origWriteText && !t.includes('handleAuth')) || (origWrite && !w.includes('handleAuth'))) hookAPI();
            }
            if (originalExec && !document.execCommand.toString().includes('handleAuth')) hookExec();
        } catch (e) { console.log('自修复执行失败: 接口可能被页面硬锁定'); }
    };
    ...
    const init = () => { 
      const authMode = (typeof GM_getValue !== 'undefined' ? GM_getValue('clipboard_auth_mode_' + location.host, 0) : 0);
      const unlockMode = (typeof GM_getValue !== 'undefined' ? GM_getValue('clipboard_unlock_mode_' + location.host, 1) : 1);
        if (unlockMode !== 0) toggleUnlock(unlockMode);
        if (authMode === 0) { hookAPI(); hookExec(); enabled = true; freeCopyMode = false; }
        else if (authMode === 2) { restoreAllCopyAPIs(); enabled = true; freeCopyMode = true; }
        else { enabled = false; freeCopyMode = false; disableAllCopyAPIs(); }
    }; init();
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => setTimeout(init, 0));
    else setTimeout(init, 0);
    document.addEventListener('mousedown', checkAndFix, { capture: true, passive: true });
    document.addEventListener('keydown', checkAndFix, { capture: true, passive: true });
    document.addEventListener('touchstart', checkAndFix, { capture: true, passive: true });
})();