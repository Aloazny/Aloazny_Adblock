...
(function() {
  ...
  const setLink = function(node) {
    ...
    while ((match = urlRegex.exec(content)) !== null) {
      ...
      let isEmail = match[1] !== undefined;
      let isSpecialProto = match[4] || match[5] || match[6];
      if (!isEmail && !isSpecialProto) {
        let prefix = "";
        if (offset === 0 && node.previousSibling) {
          let preNode = node.previousSibling;
          let preMatch = preNode.textContent.match(/[a-z0-9]+$/i);
          if (preMatch) prefix = preMatch[0];
        } else if (offset > 0) {
          let preMatch = content.substring(0, offset).match(/[a-z0-9]+$/i);
          if (preMatch) {
            prefix = preMatch[0];
