...
(function() {
    ...
    const existingHost = document.getElementById('mb-debug-host');
    if (existingHost) existingHost.remove();
    const host = document.createElement('div');
    host.id = 'mb-debug-host';
    host.setAttribute('aria-hidden', 'true');
    host.setAttribute('data-html2canvas-ignore', 'true');
    host.setAttribute('data-savepage-ignore', 'true');
    host.style.cssText = 'position:absolute;top:0;left:0;width:0;height:0;z-index:2147483647;display:block !important;';
    document.documentElement.appendChild(host);
    const shadow = host.attachShadow({ mode: 'closed' });

    const safeWrapper = document.createElement('div');
    safeWrapper.id = 'mb-safe-wrapper';
    shadow.appendChild(safeWrapper);
    ...
    api.addStyle(`
        @media print, screen and (max-width: 1px), min-resolution: 0.001dpi { :host, #mb-safe-wrapper, #mb-debug-panel, #mb-debug-trigger { display: none !important; opacity: 0 !important; visibility: hidden !important; height: 0 !important; width: 0 !important; overflow: hidden !important; position: absolute !important; left: -9999px !important; } }
        .mb-tool-btn[data-text]::before, .ad-mini-btn[data-text]::before, .edit-btn[data-text]::before, .data-key-label[data-text]::before, .icon-config-row span[data-text]::before, #mb-editor-header span[data-text]::before, #mb-code-title[data-text]::before, .data-group-box span[data-text]::before, .mb-net-msg[data-text]::before, .data-group-box div[data-text]::before { content: attr(data-text) !important; }
        :host {
        ...
        #mb-js-input { flex-shrink: 0; height: 100px; background: var(--mb-bg); color: var(--mb-text); border: 1px solid var(--mb-border); padding: 8px; font-size: 13px; font-family: monospace; overflow-y: auto; outline: none; white-space: pre-wrap; word-break: break-all; }
        #mb-js-input:empty::before { content: attr(data-placeholder); color: #666; pointer-events: none; }
    ...
    globalStyle.textContent = `
        @media print, screen and (max-width: 1px), min-resolution: 0.001dpi { #mb-debug-host { display: none !important; opacity: 0 !important; visibility: hidden !important; height: 0 !important; width: 0 !important; overflow: hidden !important; position: absolute !important; left: -9999px !important; } }
        #mb-debug-host:empty { display: none !important; }
    ...
    panel.innerHTML = `
        <div id="mb-debug-header">
            <div class="mb-header-left">
                <span class="mb-tool-btn" id="mb-btn-pick" data-text="🎯选取"></span>
            </div>
            <div class="mb-header-middle">
                <span class="mb-tool-btn" id="mb-btn-fold" data-text="▼收起"></span>
                <span class="mb-tool-btn" id="mb-btn-back" data-text="⬅ 返回" style="display:none;"></span>
                <span class="mb-tool-btn" id="mb-btn-parent" data-text="父"></span>
                <span class="mb-tool-btn" id="mb-btn-restore" data-text="恢复"></span>
                <span class="mb-tool-btn" id="mb-btn-to-ad" data-text="AD规则" style="color:#d11; font-weight:bold;"></span>
                <span class="mb-tool-btn" id="mb-btn-to-js" data-text="JS代码" style="color:#f1c40f; font-weight:bold;"></span>
                <span class="mb-tool-btn" id="mb-btn-to-net" data-text="网络" style="color:#0fb9b1; font-weight:bold;"></span>
                <span class="mb-tool-btn" id="mb-btn-to-data" data-text="🛡️数据" style="color:#009432; font-weight:bold;"></span>
                <span class="mb-tool-btn" id="mb-btn-copy-html" data-text="📋复制"></span>
                <span class="mb-tool-btn" id="mb-btn-to-icon" data-text="🧩图标" style="color:#007aff; font-weight:bold;"></span>
            </div>
            <div class="mb-header-right">
                <span class="mb-tool-btn" id="mb-btn-close" data-text="✕"></span>
            </div>
        </div>
        ...
        <div id="mb-main-stage">
            <div class="mb-page" id="page-dom" style="padding:0;">
                <div id="mb-debug-content">
                    ...
                    <div id="mb-node-actions">
                        <button class="edit-btn" id="btn-edit-html" data-text="🏗️代码模式"></button>
                        <button class="edit-btn" id="btn-edit-node" data-text="📝文字模式"></button>
                        <button class="edit-btn" id="btn-edit-img" data-text="🖼️换图" style="display:none;"></button>
                        <button class="edit-btn" id="btn-del-node" data-text="✂️删除" style="color:#e74c3c;"></button>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-page" id="page-js">
                <div id="mb-js-content">
                    <div id="mb-js-input" contenteditable="true" data-placeholder="输入 JS 代码..."></div>
                    <div class="ad-action-bar" style="margin-top:8px;">
                        <button class="ad-mini-btn" id="btn-js-run" data-text="执行" style="background:#f1c40f; color:#000; border:none; font-weight:bold;"></button>
                        <button class="ad-mini-btn" id="btn-js-clear" data-text="清空日志"></button>
                        <button class="ad-mini-btn" id="btn-js-copy-all" data-text="复制日志"></button>
                        <button class="ad-mini-btn" id="btn-log-switch" data-text="监听网页日志"></button>
                    </div>
            ...
            <div class="mb-page" id="page-copy-options">
                <div id="mb-copy-content" style="padding:15px; display:grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap:12px;">
                    <div class="data-group-box" id="copy-box-html" style="cursor:pointer; text-align:center; padding:20px;">
                        <div style="font-size:24px; margin-bottom:8px;" data-text="📄"></div>
                        <span style="display:block; font-weight:bold;" data-text="复制 HTML"></span>
                        <span style="display:block; font-size:10px; opacity:0.6; margin-top:4px;" data-text="原始 code"></span>
                    </div>
                    <div class="data-group-box" id="copy-box-text" style="cursor:pointer; text-align:center; padding:20px;">
                        <div style="font-size:24px; margin-bottom:8px;" data-text="📝"></div>
                        <span style="display:block; font-weight:bold;" data-text="提取文字"></span>
                        <span style="display:block; font-size:10px; opacity:0.6; margin-top:4px;" data-text="排版去空格"></span>
                    </div>
                    <div class="data-group-box" id="copy-box-xpath" style="cursor:pointer; text-align:center; padding:20px;">
                        <div style="font-size:24px; margin-bottom:8px;" data-text="📍"></div>
                        <span style="display:block; font-weight:bold;" data-text="复制 XPath"></span>
                        <span style="display:block; font-size:10px; opacity:0.6; margin-top:4px;" data-text="元素路径"></span>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-page" id="page-code">
                <div id="mb-debug-header-code" style="display:flex; align-items:center; background:var(--mb-header-bg); height:40px; border-bottom:1px solid var(--mb-border);">
                    <div class="mb-header-left"><span class="mb-tool-btn" id="mb-btn-code-back" data-text="⬅ 返回"></span></div>
                    <div class="mb-header-middle"><span id="mb-code-title" data-text="代码查看" style="font-size:12px;opacity:0.8;"></span></div>
                    <div class="mb-header-right"><span class="mb-tool-btn" id="mb-btn-code-copy" data-text="复制全部"></span></div>
                </div>
            ...
            <div class="mb-page" id="page-editor">
                <div id="mb-editor-header" style="display:flex; align-items:center; background:var(--mb-header-bg); height:40px; border-bottom:1px solid var(--mb-border); flex-shrink:0;">
                    <div class="mb-header-left"><span class="mb-tool-btn" id="mb-btn-editor-back" data-text="⬅ 返回"></span></div>
                    <div class="mb-header-middle"><span data-text="HTML 自由编辑模式" style="font-size:12px;opacity:0.8;"></span></div>
                    <div class="mb-header-right"><span class="mb-tool-btn" id="mb-btn-editor-save" data-text="完成同步" style="color:#2ecc71; font-weight:bold;"></span></div>
                </div>
    ...
    `;
    safeWrapper.appendChild(panel);

    const trigger = document.createElement('div');
    trigger.id = 'mb-debug-trigger';
    safeWrapper.appendChild(trigger);
    ...
    if (jsInput) jsInput.placeholder = "输入 JS 代码...";
    ...
    function updateLogBtnUI() {
        if (isLogging) {
            ...
            btnLogSwitch.setAttribute('data-text', '停止监听');
        } else {
            btnLogSwitch.style.background = 'var(--mb-bg)';
            btnLogSwitch.style.color = 'var(--mb-text)';
            btnLogSwitch.setAttribute('data-text', '监听网页日志');
        }
    }
    ...
    function updateFoldState(action) {
      const foldBtn = shadow.getElementById('mb-btn-fold');
        if (action === 'hide') { isCollapsed = true; panel.style.height = '0'; panel.style.pointerEvents = 'none'; document.body.style.paddingBottom = '0';
        if (foldBtn) foldBtn.setAttribute('data-text', '▲展开');
        const mainStage = shadow.getElementById('mb-main-stage');
        ...
        const mainStage = shadow.getElementById('mb-main-stage');
        if (mainStage) mainStage.style.display = isCollapsed ? 'none' : 'flex';
    }
    ...
    */
    function generateSmartRules(el) {
        ...
        let advancedRules = [];
        if (el.previousElementSibling && chromeVersion >= 88) {
            const prev = el.previousElementSibling;
    ...
    function renderLinksPage() {
        const linksContent = shadow.getElementById('mb-links-content');
        if (!linksContent) return;
        linksContent.innerHTML = '';
        if (!currentTarget) {
            linksContent.innerHTML = '<div style="color:#999;padding:20px;">请先在网页上点击选取一个元素...</div>';
            return;
        }
        const linkEls = [];
        ...
        const getTypeTag = (url) => {
            try {
                ...
                if (!pathname.includes('.') || ['html', 'htm', 'shtml', 'php', 'jsp', 'asp', 'aspx'].includes(ext)) {
                    return { label: 'link', color: '#6c757d' };
                }
                if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'ico', 'bmp'].includes(ext)) return { label: ext, color: '#28a745' };
                if (['mp4', 'm3u8', 'webm', 'flv', 'avi', 'mov', 'm4v', 'mkv'].includes(ext)) return { label: ext, color: '#dc3545' };
                if (['mp3', 'wav', 'aac', 'flac', 'ogg', 'm4a'].includes(ext)) return { label: ext, color: '#fd7e14' };
                if (['pdf', 'zip', 'rar', '7z', 'tar', 'gz', 'doc', 'docx', 'xls', 'xlsx'].includes(ext)) return { label: ext, color: '#17a2b8' };
                return { label: ext, color: '#007bff' };
            } catch (e) {
                return { label: 'link', color: '#6c757d' };
            }
        };
        linkEls.forEach(el => {
            ...
            const originalRule = absUrl;
            const item = document.createElement('div');
            item.className = 'ad-rule-item';
            const updateUI = (isEditing = false) => {
                const typeInfo = getTypeTag(currentRule);
                const tagHtml = `<span style="display:inline-block; padding:1px 5px; margin-right:5px; border-radius:3px; background:${typeInfo.color}; color:#fff; font-size:11px; font-weight:bold; text-transform:lowercase;">[${typeInfo.label}]</span>`;
                item.innerHTML = `
                    <div class="ad-rule-display" ${isEditing ? 'contenteditable="true" style="border:1px solid #007bff; padding:5px; outline:none; background:var(--mb-bg); word-break:break-all; white-space:normal;"' : 'style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"'}>${isEditing ? currentRule : `${tagHtml}<span class="hl-url">${currentRule}</span>`}</div>
                    <div class="ad-action-bar">
                        ${isEditing ? `
                            <button class="ad-mini-btn btn-save" style="background:#28a745; color:#fff; border:none;">保存</button>
                            <button class="ad-mini-btn btn-undo">撤销</button>
                        ` : `
                            ...
                            <button class="ad-mini-btn btn-open">打开</button>
                            <button class="ad-mini-btn btn-edit">编辑</button>
                        `}
                    </div>
                `;
    ...
    async function renderNetworkPage() {
        ...
        if (currPage === 'detail' && netState.currentLog) {
            ...
            if (l.source === 'resource') {
                ...
                const isImgPath = /\.(jpg|jpeg|png|gif|webp|svg|ico|bmp|avif|heic)$/i.test(urlNoQuery) || l.resType?.includes('img');
                ...
                const fetchAndCheck = async () => {
                    const fetchRes = (text) => {
                        ...
                        preview.innerHTML = `<button class="ad-mini-btn" id="copy-res-pre" style="float:right;zoom:0.8;margin-top:-22px;background:var(--mb-border);">复制</button><pre style="font-size:11px;overflow:auto;max-height:350px;background:#000;color:#fff;padding:8px;white-space:pre-wrap;">${formatAndHighlight(text, (l.url.includes('.css')?'css':(isLikelyJS?'js':'text')))}</pre>`;
                    ...
                    if (isFont) {
                        ...
                        preview.innerHTML = `<style>@font-face { font-family: '${fontId}'; src: url('${l.url}?t=${Date.now()}'); }.font-preview-box { font-family: '${fontId}', sans-serif; font-size: 16px; line-height: 1.6; color: var(--mb-text); background: var(--mb-header-bg); padding: 15px; border-radius: 4px; border: 1px solid var(--mb-border); text-align: left; word-break: break-all; }</style><div class="font-preview-box"><div style="font-size:10px;color:var(--mb-active);margin-bottom:8px;font-family:sans-serif;">字体预览 (十问歌):</div>一问寒热二问汗，三问头身四问便，五问饮食，六问胸，七聋八渴俱当辨，九问旧病十问因，再兼服药参机变，妇女尤必问经期，迟速闭崩皆可见，再问片语告儿科，天花麻疹全占验。</div><div style="margin-top:10px;font-size:11px;opacity:0.6;word-break:break-all;">URL: ${esc(l.url)}</div>`;
                        return;
                    }
                    const handleBuffer = (buffer) => {
                        const arr = new Uint8Array(buffer);
                        const hex = Array.from(arr.slice(0, 12)).map(b => b.toString(16).padStart(2, '0')).join('');
                        const isRealImg = /89504e47|ffd8ff|47494638|424d|00000100|00000200/.test(hex.slice(0, 8)) || (hex.startsWith('52494646') && hex.includes('57454250')) || hex.includes('6674797061766966') || hex.includes('6674797068656963');
                        const isSvg = hex.startsWith('3c737667') || hex.startsWith('3c3f786d') || hex.startsWith('3c21444f') || hex.startsWith('efbbbf3c');
                        if (isImgPath && (isRealImg || isSvg)) {
                            let mime = '';
                            if (isSvg) mime = 'image/svg+xml';
                            else if (hex.startsWith('00000100')) mime = 'image/x-icon';
                            else if (hex.startsWith('424d')) mime = 'image/bmp';
                            else if (hex.includes('6674797061766966')) mime = 'image/avif';
                            ...
                            const blob = mime ? new Blob([arr], { type: mime }) : new Blob([arr]);
                            const imgUrl = URL.createObjectURL(blob);
                            preview.innerHTML = `<img src="${imgUrl}" style="max-width:100%;max-height:300px;border:1px solid var(--mb-border);" onload="URL.revokeObjectURL('${imgUrl}')" onerror="URL.revokeObjectURL('${imgUrl}')">`;
                        } else {
                    ...
                    if (typeof GM_xmlhttpRequest !== 'undefined') {
                        GM_xmlhttpRequest({ method: "GET", url: l.url, responseType: "arraybuffer", headers: { "x-mb-is-script": "true" }, onload: (res) => {
                            handleBuffer(res.response);
                        }, onerror: () => { preview.innerHTML = '<span style="color:#ff4d4f">Fetch Failed</span>'; } });
                    } else {
                        try { 
                            const res = await fetch(l.url, { method: 'GET', headers: { "x-mb-is-script": "true" }, credentials: 'omit', cache: 'default' }); 
                            const buffer = await res.arrayBuffer();
                            handleBuffer(buffer);
                        } catch(e) { 
                            if (isImgPath) {
                                preview.innerHTML = `<img src="${l.url}" style="max-width:100%;max-height:300px;border:1px solid var(--mb-border);" onerror="this.parentNode.innerHTML='<span style=\'color:#ff4d4f\'>CORS Blocked</span>'">`;
                            } else {
                                preview.innerHTML = `<span style="color:#ff4d4f">CORS Blocked</span><br><small style="font-size:10px;opacity:0.6;word-break:break-all;">${e.message}</small>`;
                            }
                        }
                    }
                };
    ...
    function renderSearchUI(bar) {
        bar.innerHTML = `
            <div style="display:flex; flex-direction:column; width:100%; gap:8px;" id="mb-search-container">
                <div style="display:flex; align-items:center; gap:5px; width:100%;">
                    ...
                    <input type="text" id="mb-search-input" placeholder="关键词 / CSS / Adblock规则..." style="flex:1; height:30px; padding:0 8px; border:1px solid var(--mb-border); background:var(--mb-bg); color:var(--mb-text); border-radius:4px; outline:none; font-size:13px;">
        ...
        const updateSelection = () => {
            if (searchResults.length > 0) {
                ...
                if (treeContainer) {
                    ...
                    const treeNode = buildTree(currentTarget, true);
                    treeContainer.appendChild(treeNode);
                    setTimeout(() => {
                        const targetLine = treeContainer.querySelector('.node-row.selected');
                        if (targetLine) {
                            targetLine.scrollIntoView({ behavior: 'auto', block: 'center', inline: 'nearest' });
                        }
                    }, 50);
                }
        ...
        const doSearch = () => {
            ...
            if (!rawVal) { countLab.innerText = '请输入内容'; return; }
            input.blur(); 
            searchResults = [];
            currentSearchIdx = 0;
            ...
            setTimeout(() => {
                ...
                allElements.forEach(el => {
                    ...
                    const matchTag = el.tagName.toLowerCase().includes(searchVal);
                    const matchText = Array.from(el.childNodes).some(n => n.nodeType === 3 && n.textContent.toLowerCase().includes(searchVal));
                    const matchAttr = Array.from(el.attributes).some(a => a.name.toLowerCase().includes(searchVal) || a.value.toLowerCase().includes(searchVal));
    ...
    function updateNodeActions() {
        ...
        const btnLinks = document.createElement('button'); btnLinks.className = 'edit-btn'; btnLinks.id = 'btn-extract-links'; btnLinks.style.cssText = 'color:#e67e22; border-color:#e67e22;'; btnLinks.innerText = '🔗提取链接'; btnLinks.onclick = (e) => { e.stopPropagation(); renderLinksPage(); switchToPage(9); }; actionsBar.appendChild(btnLinks);
    ...
    function autoInitAndHide() {
        const host = panel.getRootNode().host || panel;
        const origHostVisibility = host.style ? host.style.visibility : '';
        const origPanelVisibility = panel.style.visibility;
        if (host.style) host.style.setProperty('visibility', 'hidden', 'important');
        panel.style.setProperty('visibility', 'hidden', 'important');
        try {
            togglePanel(true);
            togglePanel(false);
        } catch (e) {
            console.error('[M-Browser] 自动化预加载任务执行出错: ', e);
        } finally {
            if (host.style) host.style.visibility = origHostVisibility;
            panel.style.visibility = origPanelVisibility;
            console.log('[M-Browser] 脚本界面自动预加载与清理任务已圆满完成。');
        }
    }
    ...
    const linksPage = document.createElement('div'); linksPage.className = 'mb-page'; linksPage.id = 'page-links'; linksPage.innerHTML = '<div id="mb-links-content" style="flex:1; overflow:auto; padding:10px; background:var(--mb-bg) !important;"></div>';
    ...
    shadow.getElementById('btn-js-clear').onclick = () => { jsLog.innerHTML = ''; jsInput.innerText = ''; };
    shadow.getElementById('btn-js-run').onclick = () => { const code = jsInput.innerText.trim(); if (!code) return; try { const result = window.eval(code); if (result !== undefined) addLog(result, 'log-result'); } catch (e) { addLog(e.stack || e.message, 'log-error'); }};
