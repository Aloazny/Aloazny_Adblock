...
(function() {
    ...
    function checkUrl(s) {
        const protocolMatch = s.match(PROTOCOL_PREFIX_REG);
        if (!protocolMatch && !s.includes('.')) return null;
        const urlPath = s.split(/[?#]/)[0];
        if (FILE_EXTS_REG.test(urlPath) && !protocolMatch) return null;
        const hasProtocol = /^(https?:\/\/|ftp:\/\/)/i.test(s);
        const domain = s.replace(PROTOCOL_PREFIX_REG, '').split(/[/?#]/)[0].toLowerCase();
        const parts = domain.split('.');
        const tld = parts[parts.length - 1];
        if (TLD_MATCH_REG.test(tld)) return hasProtocol ? s : (s.toLowerCase().startsWith('www.') ? 'https://' + s : 'https://' + s);
        return hasProtocol ? s : null;
    }
