...
(function() {
    ...
    function createUI() {
        ...
        style.textContent = `
            ...
            .url-text { font-size: 11px; color: #888; white-space: nowrap; overflow: hidden; flex: 1; mask-image: linear-gradient(to right, black 85%, transparent 100%); }
            .url-text:hover { overflow: visible; animation: scroll 8s linear infinite; }
            ...
            .btn { border:none; padding:6px 12px; border-radius:8px; cursor:pointer; font-size:12px; transition:0.2s; background:#eee; }
            .btn-mode.active { background:#007AFF; color:#fff; }
            .btn-toggle.on { background:#34C759; color:#fff; }
            ...
            .import-btn { background:#34C759; color:#fff; border:none; padding:8px; border-radius:10px; font-weight:bold; cursor:pointer; margin-top:5px; }
            ...
            .conflict-guide { font-size: 12px; opacity: 0.8; margin-top: 10px; border-top: 1px solid rgba(255,255,255,0.3); padding-top: 10px; }
        ...
        const render = () => {
            panel.innerHTML = `
                <div class="header">
                    ...
                    <div class="url-bar">
                        <div class="url-text"><span>${location.href}</span></div>
                ...
                <div id="conflict-layer" class="conflict-layer">
                    ...
                    <div class="footer" style="width:100%">
                        ...
                        <button class="btn" style="background:transparent;color:#fff;border:1px solid #fff" onclick="this.parentElement.parentElement.classList.remove('show')">忽略并继续</button>
                    </div>
                </div>
                <div class="footer">
            ...
            if (action === 'toggle') {
                ...
                try {
                    ...
                    Object.keys(defaultSettings).forEach(k => {
                        if (parsed.hasOwnProperty(k)) safeSettings[k] = parsed[k];
                        else safeSettings[k] = defaultSettings[k];
                    });
                ...
                if (settings.newTab.enabled && settings.removeNewTab.enabled && settings.newTab.mode === settings.removeNewTab.mode) {
                    ...
                    if (intersection.length > 0) {
                        handleConflict(`规则 [${intersection[0]}] 在"新标签"与"禁新标"中存在逻辑冲突。`, 1, 'newTab');
                        return;
                    }
                }
        ...
        panel.addEventListener('blur', (e) => {
            if (e.target.tagName === 'TEXTAREA' && e.target.id !== 'io-area') {
                const el = e.target;
                ...
                for (let line of lines) {
                    if (line.startsWith('/') && line.endsWith('/')) {
                        try { new RegExp(line.slice(1, -1)); } catch(err) { isValid = false; break; }
                    }
                }
