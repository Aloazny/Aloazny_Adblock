...
(function() {
    ...
    function blockAds() {
        ...
        ['#btimgid1', '#btmad1', 'script[src$="/js/jads.js"] + ins[id]', 'iframe[width="300"][height="250"]', 'iframe[src*="/smartpop/"]', 'iframe[src*="/herebyad"]', 'iframe[src*="/HereByAD"]', 'iframe[style*="300"][style*="250"]', 'div[style*="z-index: 9999"][style*="justify-content: center;"]:not([class]):not([id])', 'a[href][target="_blank"] > img[src^="//t4"][src*="/data/t/"]', 'a[href][target="_blank"] > img[src*="/game/"]'].forEach(s => {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (target && target.href) {
                try {
                    ...
                    const isInternal = url.hostname.includes('wnacg') || url.hostname.includes('www.wn') || url.hostname.includes(fuzzyDomain);
    ...
    function handleNormalPage() {
        document.querySelectorAll('.dv-btns, .hlol_ad, .download_btn, .btn-row-left, div[id*="ad"], div[class*="ad"]').forEach(container => {
            ...
            if (container.classList.contains('dv-btns') || container.classList.contains('btn-row-left')) {
                const adBtn = container.querySelector('a.ad-site');
                if (adBtn) {
                    const newBtn = document.createElement('a');
                    newBtn.className = 'dv-btn-icon';
                    newBtn.title = '下載漫畫';
                    newBtn.style.cssText = 'cursor:pointer; background:#4CAF50 !important; color:white !important; margin-left:5px; display:flex; align-items:center; justify-content:center;';
                    newBtn.innerHTML = `<svg width="19" height="19" viewBox="0 0 24 24" fill="currentColor"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"></path></svg>`;
                    newBtn.onclick = (e) => {
                        e.preventDefault();
    ...
    function handleDownloadPage() {
        if (typeof unsafeWindow !== 'undefined') {
            unsafeWindow.initializeDownloadEnvironment = function() { return false; };
            unsafeWindow.lockdownInterface = function() { return false; };
        }
        window.initializeDownloadEnvironment = function() { return false; };
        window.lockdownInterface = function() { return false; };
