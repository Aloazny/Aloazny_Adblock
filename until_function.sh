...
Adblock_Tools_Plugin_Folder="$(pwd)"
#调用magisk的busybox sed
...
key
write_notran_file "${original_file}" "${file}" ">>"
...
local file="${1}"
local white_list=`grep -E '^#\@#' "${file}" | busybox sed -E 's/#\@#/##/g' `
for i in ${white_list}
do
	echo "剔除冲突规则 ${i}"
...
local file="${1}"
test ! -f "${file}" && return
busybox sed '/^\!/d;/\#/d;/\$/d' "${file}" | grep -E '^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}(:[0-9]{1,5})?(/[^ ]*)?' | sort -u | while read line
do
...
local file="${1}"
test ! -f "${file}" && return
grep -E 'domain=~' "${file}" | busybox sed '/#/d;s/\$.*//g' | while read line
do
...
do
	...
	[ -n "${_cf_cunt_transfer}" ] && echo "${_cf_cunt_transfer}" || echo "0"
done
}
...
> ### 补充规则 ( $(count_filter_files "`pwd`/Rules/Adblock_attach.txt") )
<details>
...
<li> <a href="https://cdn.jsdelivr.net/gh/Aloazny/Aloazny_Adblock@main/Rules/Adblock_attach.txt" > Jsdelivrcdn缓存链接 </a> </li>
...
> ### 中文规则 ( $(count_filter_files "`pwd`/Rules/Adblock_Chinese.txt") )
<details>
...
<li> <a href="https://cdn.jsdelivr.net/gh/Aloazny/Aloazny_Adblock@main/Rules/Adblock_Chinese.txt" > Jsdelivrcdn缓存链接 </a> </li>
...
> ### 移动端规则 ( $(count_filter_files "`pwd`/Rules/Adblock.txt") )
<details>
...
<li> <a href="https://cdn.jsdelivr.net/gh/Aloazny/Aloazny_Adblock@main/Rules/Adblock.txt" > Jsdelivrcdn缓存链接 </a> </li>
